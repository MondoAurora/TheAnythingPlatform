...
//				Map params = new HashMap();
				...
				Dust.access(DustAccess.Set, url, hRelay, TOKEN_STREAM_ATT_URL);
				Dust.access(DustAccess.Set, pathInfo, hRelay, TOKEN_NET_ATT_SRVCALL_PATHINFO);

				Dust.access(DustAccess.Set, request, hRelay, TOKEN_NET_ATT_SRVCALL_REQUEST);
				Dust.access(DustAccess.Set, response, hRelay, TOKEN_NET_ATT_SRVCALL_RESPONSE);

				Dust.access(DustAccess.Set, request.getMethod(), hRelay, TOKEN_NET_ATT_SRVCALL_METHOD);
				...
				for (ee = request.getAttributeNames(); ee.hasMoreElements();) {
					n = ee.nextElement();
					optAdd(hRelay, TOKEN_NET_ATT_SRVCALL_ATTRIBUTES, n, request.getAttribute(n));
				}

				for (ee = request.getParameterNames(); ee.hasMoreElements();) {
					n = ee.nextElement();
					optAdd(hRelay, TOKEN_MISC_ATT_PAYLOAD, n, request.getParameter(n));
				}

				for (ee = request.getHeaderNames(); ee.hasMoreElements();) {
					n = ee.nextElement();
					optAdd(hRelay, TOKEN_NET_ATT_SRVCALL_HEADERS, n, request.getHeader(n));
				}

				String cmd = Dust.access(DustAccess.Peek, null, hRelay, TOKEN_MIND_ATT_CMD);
				if (null == cmd) {
					cmd = DustUtils.getPrefix(pathInfo, "/");
					Dust.access(DustAccess.Set, cmd, hRelay, TOKEN_MIND_ATT_CMD);
				}
...
//				params = DustUtilsData.optLoadMapping(hRelay, params);
				...
				Dust.access(DustAccess.Process, null, hRelay);

				int status = Dust.access(DustAccess.Peek, HttpServletResponse.SC_OK, hRelay, TOKEN_MISC_ATT_TARGET, TOKEN_NET_ATT_SRVCALL_STATUS);
