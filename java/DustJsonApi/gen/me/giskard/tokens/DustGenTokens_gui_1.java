package me.giskard.tokens;

// Generation timestamp 20260818T092043Z

public interface DustGenTokens_gui_1 {
// Aspects
	String TOKEN_GUI_ASP_EDIT_COLL = "gui.1$EditColl";
	String TOKEN_GUI_ASP_EDIT_HANDLE = "gui.1$EditHandle";
	String TOKEN_GUI_ASP_EDIT_NUMBER = "gui.1$EditNumber";
	String TOKEN_GUI_ASP_EDIT_OPTION = "gui.1$EditOption";
	String TOKEN_GUI_ASP_EDIT_TEXT = "gui.1$EditText";
	String TOKEN_GUI_ASP_PANEL_CALENDAR = "gui.1$PanelCalendar";
	String TOKEN_GUI_ASP_PANEL_CONTAINER = "gui.1$PanelContainer";
	String TOKEN_GUI_ASP_PANEL_IDEA_GRID = "gui.1$PanelIdeaGrid";
	String TOKEN_GUI_ASP_PANEL_IDEA_PROPERTIES = "gui.1$PanelIdeaProperties";
	String TOKEN_GUI_ASP_PANEL_MEDIA_PLAYER = "gui.1$PanelMediaPlayer";
	String TOKEN_GUI_ASP_PANEL_SPLIT = "gui.1$PanelSplit";
	String TOKEN_GUI_ASP_PANEL_TOOLBAR = "gui.1$PanelToolbar";
	String TOKEN_GUI_ASP_WIDGET_BUTTON = "gui.1$WidgetButton";
	String TOKEN_GUI_ASP_WIDGET_LABEL = "gui.1$WidgetLabel";
	String TOKEN_GUI_ASP_WIDGET_PROGRESS = "gui.1$WidgetProgress";
	String TOKEN_GUI_ASP_WINDOW = "gui.1$Window";

// Attributes
	String TOKEN_GUI_ATT_IMAGE_RESOLVER = "gui.1$imageResolver";
	String TOKEN_GUI_ATT_LOOKANDFEEL = "gui.1$lookAndFeel";

// Tags
	String TOKEN_GUI_TAG_AXISUNIT_PIXEL = "gui.1$AxisUnitPixel";
	String TOKEN_GUI_TAG_AXIS_SCREEN_X = "gui.1$AxisScreenX";
	String TOKEN_GUI_TAG_AXIS_SCREEN_Y = "gui.1$AxisScreenY";
	String TOKEN_GUI_TAG_NUMBER_MODE = "gui.1$NumberMode";
	String TOKEN_GUI_TAG_NUMBER_MODE_SLIDER = "gui.1$NumberModeSlider";
	String TOKEN_GUI_TAG_NUMBER_MODE_SPINNER = "gui.1$NumberModeSpinner";
	String TOKEN_GUI_TAG_NUMBER_MODE_TEXT = "gui.1$NumberModeText";
	String TOKEN_GUI_TAG_OPTION_MODE = "gui.1$OptionMode";
	String TOKEN_GUI_TAG_OPTION_MODE_COMBO = "gui.1$OptionModeCombo";
	String TOKEN_GUI_TAG_OPTION_MODE_LIST = "gui.1$OptionModeList";
	String TOKEN_GUI_TAG_OPTION_MODE_RADIO = "gui.1$OptionModeRadio";
	String TOKEN_GUI_TAG_OPTION_MODE_SLIDER = "gui.1$OptionModeSlider";
	String TOKEN_GUI_TAG_OPTION_MODE_TOGGLE = "gui.1$OptionModeToggle";
	String TOKEN_GUI_TAG_TEXT_MODE = "gui.1$TextMode";
	String TOKEN_GUI_TAG_TEXT_MODE_FORMATTED = "gui.1$TextModeFormatted";
	String TOKEN_GUI_TAG_TEXT_MODE_MULTI_LINE = "gui.1$TextModeMultiLine";
	String TOKEN_GUI_TAG_TEXT_MODE_SINGLE_LINE = "gui.1$TextModeSingleLine";
}
